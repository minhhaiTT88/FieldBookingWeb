...
const Index = () => {
  ...
  const [fieldBookingList, setFieldBookingList] = useState();
  ...
  useEffect(() => {
    ...
    setFieldId(_id);
    loadFieldBookingInDay(_id);
  }, [searchParams]);

  const loadFieldBookingInDay = (field_id) => {
    var keysearch = `${field_id}||||${moment().format(
      "YYYY-MM-DD"
    )}|${moment().format("YYYY-MM-DD")}`;
    apiFieldBooking
      .SearchData(keysearch, 1, 1, "BookingDate DESC")
      .then((res) => {
        if (res && res?.jsondata) {
          console.log(res);
          var lst = JSON.parse(res?.jsondata);
          console.log(lst);
          gridRef.current.api.setRowData(lst);
          setFieldBookingList(lst);
        }else{
          setFieldBookingList([]);
        }
      });
          ...
          <div className="flex-content">
            <div className="card-content">
              <Form
                ...
                // }}
              >
                <ItemSearch ShowModal={ShowModal} />
              </Form>
            </div>
